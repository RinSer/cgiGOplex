...
/**
 * CGI-script that works as a server creating html pages 
 * with specified Julia set images.
 * 
 * created by RinSer
 */
...
func viewHandler(w http.ResponseWriter, r *http.Request) {
    ...
    // ! Set the html template file path !
    path_to_template := "/home/httpd/vhosts/finengin.net/httpdocs/goplex/julia.html"
    html, err := template.ParseFiles(path_to_template)
    if err != nil {
        panic(err)
    }
    // Screen Resolution
    if len(params) > 3 {
        ...
        c = complex(rec, imc)
        // Create image and html template
